...
//opensaga.fun_account_insert_update(IN inidaccount integer, IN inpartition integer, IN inenable boolean, IN inaccount text, IN inname text, IN intype integer, IN innote text, OUT outidaccount integer, OUT outpgmsg text)
...
Rows->add_child(EventViewNodeXml(r).Row());
}
...
Fila.addFieldInt("idevent", event.IdEvent);
Fila.addFieldString("dateload", event.DateLoad, true);
Fila.addFieldInt("idaccount", event.IdAccount);
Fila.addFieldInt("partition", event.Partition);
Fila.addFieldString("description", event.Description, true);
Fila.addFieldBool("enable", event.Enable);
Fila.addFieldString("account", event.Account, true);
Fila.addFieldString("name", event.Name, true);
Fila.addFieldInt("ideventtype", event.IdEvenType);
Fila.addFieldString("code", event.Code, true);
Fila.addFieldInt("zu", event.ZU);
Fila.addFieldInt("priority", event.Priority);
Fila.addFieldString("eventtype", event.EventType, true);
Fila.addFieldString("datetimeevent", event.Date, true);

Fila.addFieldInt("process1", event.Process1);
Fila.addFieldString("dateprocess1", event.DateProcess1, true);

Fila.addFieldInt("process2", event.Process1);
Fila.addFieldString("dateprocess2", event.DateProcess1, true);

Fila.addFieldInt("process3", event.Process1);
Fila.addFieldString("dateprocess3", event.DateProcess1, true);

Fila.addFieldInt("process4", event.Process1);
Fila.addFieldString("dateprocess4", event.DateProcess1, true);

Fila.addFieldInt("process5", event.Process1);
Fila.addFieldString("dateprocess5", event.DateProcess1, true);
...
EventViewdb[] RetornoX = new EventViewdb[1];

var  Conexion = Postgres.connect_db (this.ConnString());

if(Conexion.get_status () == ConnectionStatus.OK){

var Resultado = Conexion.exec_params ("SELECT * FROM opensaga.view_events ORDER BY datetimeevent DESC LIMIT $1::integer;", valuesin.length, null, valuesin, null, null, 0);

    if (Resultado.get_status () == ExecStatus.TUPLES_OK) {

var resultrows = this.Result_FieldName(ref Resultado);
int i = 0;

RetornoX = new EventViewdb[resultrows.length];

foreach(var reg in resultrows){

EventViewdb Row =  EventViewdb();

Row.IdEvent = reg["idevent"].as_int();
Row.DateLoad = reg["dateload"].Value;
Row.IdAccount = reg["idaccount"].as_int();
Row.IdEvenType = reg["ideventtype"].as_int();
Row.Description = reg["description"].Value;
Row.Code = reg["code"].Value;
Row.ZU = reg["zu"].as_int();
Row.Priority = reg["priority"].as_int();
Row.IdEvenType = reg["ideventtype"].as_int();
Row.EventType = reg["eventtype"].Value;
Row.Date = reg["datetimeevent"].Value;
Row.Process1 = reg["process1"].as_int();
Row.DateProcess1  = reg["dateprocess1"].Value;
Row.Process2 = reg["process2"].as_int();
Row.DateProcess2 = reg["dateprocess2"].Value;
Row.Process3 = reg["process3"].as_int();
Row.DateProcess3 = reg["dateprocess3"].Value;
Row.Process4 = reg["process4"].as_int();
Row.DateProcess4  = reg["dateprocess4"].Value;
Row.Process5 = reg["process5"].as_int();
Row.DateProcess5 = reg["dateprocess5"].Value; 
...
Row.Partition = reg["partition"].as_int();
Row.Enable = reg["enable"].as_bool();
Row.Account = reg["account"].Value;
Row.Name = reg["name"].Value;
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.fun_account_phones_trigger_alarm_table($1::integer, $2::integer, $3::boolean, $4::boolean, $5::boolean, $6::text);""",  ValuesArray.length, null, ValuesArray, null, null, 0);
...
var Resultado = Conexion.exec_params ("SELECT * FROM opensaga.fun_account_users_trigger_phones_contacts($1::integer, $2::integer)", valuesin.length, null, valuesin, null, null, 0);
...
var Resultado = Conexion.exec_params ("SELECT * FROM opensaga.fun_account_contacts_table_xml($1::integer, $2::integer, $3::integer, $4::boolean, $5::text, $6::text, $7::boolean) AS return", valuesin.length, null, valuesin, null, null, 0);
...
//Query.append_printf("SELECT * FROM opensaga.fun_view_account_contact_notif_eventtypes(%i, %i, %s)", idaccount, idphone, fieldtextasbase64.to_string());
string[] valuesin = {idaccount.to_string(), idphone.to_string(), fieldtextasbase64.to_string()};

var Resultado = Conexion.exec_params ("SELECT * FROM opensaga.fun_view_account_contact_notif_eventtypes_xml($1::integer, $2::integer, $3::boolean) AS return", valuesin.length, null, valuesin, null, null, 0);
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.fun_view_account_notif_phones_xml($1::integer, $2::integer, $3::boolean) AS return""", valuesin.length, null, valuesin, null, null, 0);
...
// opensaga.fun_account_notifications_table(IN inidnotifaccount integer, IN inidaccount integer, IN inidphone integer, IN prioinrity integer, IN incall boolean, IN insms boolean, IN insmstext text, IN innote text, IN ints timestamp without time zone, OUT outreturn integer, OUT outpgmsg text)
...
var Resultado = Conexion.exec_params ("SELECT idaccount, name FROM opensaga.account ORDER BY $1::text;", valuesin.length, null, valuesin, null, null, 0);
...
var Resultado = Conexion.exec_params ("SELECT * FROM opensaga.view_account_users WHERE idaccount = $1 ORDER BY numuser, lastname, firstname", valuesin.length, null, valuesin, null, null, 0);
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.account WHERE idaccount = $1;""", valuesin.length, null, valuesin, null, null, 0);
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.fun_account_users_add($1::integer, $2::integer);""",  ValuesArray.length, null, ValuesArray, null, null, 0);
...
//opensaga.fun_account_users_table(IN inidaccount integer, IN inidcontact integer, IN inappointment text, IN inenable boolean, IN innumuser integer, IN inkeyword text, IN inpwd text, IN innote text, OUT outreturn integer, OUT outmsg text)
...
var  Conexion = Postgres.connect_db (this.ConnString());
if(Conexion.get_status () == ConnectionStatus.OK){
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.fun_account_users_table($1::integer, $2::integer, $3::text, $4::boolean, $5::integer, $6::text, $7::text, $8::text);""",  ValuesArray.length, null, ValuesArray, null, null, 0);
    if (Resultado.get_status () == ExecStatus.TUPLES_OK) {
//GLib.print("Llega hasta aqui 4 \n");
foreach(var filas in this.Result_FieldName(ref Resultado)){
//Retorno = int.parse(filas["fun_smsout_insert"]);
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.account_users WHERE idaccount=$1::integer  AND idcontact=$2::integer LIMIT 1""",  ValuesArray.length, null, ValuesArray, null, null, 0);
...
//opensaga.fun_account_location_table(IN inidaccount integer, IN ingeox real, IN ingeoy real, IN inaddress text, IN inidaddress text, IN innote text, OUT outreturn integer, OUT outpgmsg text)
...
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.fun_account_location_table($1::integer, $2::real, $3::real, $4::text, $5::text, $6::text);""",  ValuesArray.length, null, ValuesArray, null, null, 0);
...
var  Conexion = Postgres.connect_db (this.ConnString());
if(Conexion.get_status () == ConnectionStatus.OK){
var Resultado = Conexion.exec_params ("""SELECT * FROM opensaga.account_location WHERE idaccount=$1::integer LIMIT 1""",  ValuesArray.length, null, ValuesArray, null, null, 0);
