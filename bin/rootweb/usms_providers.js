...
"dojox/data/XmlStore"
	...
	ready(function() {
		...
		dNew.dijitOwner(dijit.byId('new'), 'Click').on('onok', function() {
			gridxprovider._Save( {
				idprovider: 0, enable: dijit.byId('NewEnable').get('checked'), name: dijit.byId('NewName').get('value'), note: ''
			}
			);
	...
	gridxprovider.delete = function() {
		if(gridxprovider.selected.length>0) {
			request.post('fun_provider_delete_selection_xml.usms', {
				...
				if(d.length > 0) {
					gridxprovider.emit('onnotify', {
						msg: d.getStringFromB64(0, 'outpgmsg')
					}
					);
				}
				gridxprovider._Load();
			...
			gridxprovider.emit('onnotify', {
				msg: 'No hay teléfonos seleccionados para aplicar los cambios'
			}
			);
		}
	}
