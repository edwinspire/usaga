	...
	ready(function() {
				...
				request.post('tableserialport_delete.usms', {
					...
					if(xmld.length > 0) {
						MH.notification.notify( {
							message: xmld.getStringFromB64(0, 'message')
						}
						);
					}
					GridxTable.Load();
		...
		myDialogNew.dijitOwner(dijit.byId('new'), 'Click').on('onok', function() {
			if(dijit.byId('id_port_form').validate()) {
				GridxTable.SaveItem( {
					idport: 0, baudrate: dijit.byId('id_port_baudrate').get('value'), databits: dijit.byId('id_port_databits').get('value'), enable: dijit.byId('id_port_enable').get('checked'), handshake: dijit.byId('id_port_handshake').get('value'), note: dijit.byId('id_port_note').get('value'), port: dijit.byId('id_port_port').get('value'), stopbits: dijit.byId('id_port_stopbits').get('value')
				}
				);
			} else {
				MH.notification.notify( {
					message: 'Los datos no han sido completados correctamente'
				}
				);
			}
		}
