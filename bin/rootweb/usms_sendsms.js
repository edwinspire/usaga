...
"dijit/form/Select"
	...
	ready(function() {
		...
		DialogFreeAddPhone.dijitOwner(dijit.byId('idFreeAddSender'), 'Click').on('onok', function() {
			...
			IFWS1.newItem( {
				unique_id: dijit.byId('idSMSFreeFieldPhone').get('value'), idprovider: dijit.byId('idSMSFreeFieldProvider').get('value'), provider: dijit.byId('idSMSFreeFieldProvider').displayedValue(), sim: dijit.byId('idSMSFreeFieldSIM').displayedValue(), idsim: dijit.byId('idSMSFreeFieldSIM').get('value')
			}
			);
			IFWS1.save();
		...
		DialogFreeSend.dijitOwner(dijit.byId('idFreeSendSMS'), 'Click').on('onok', function() {
			if(FormSMSFree.validate()) {
				...
				MH.notification.notify( {
					message: 'Revise que los datos del mensaje sean correctos antes de intentar nuevamente'
				}
				);
			}
		}
		...
		var PostSendSMS = function(sms_) {
			request.post('/fun_outgoing_new_xml.usms', {
				...
				if(xmld.length > 0) {
					MH.notification.notify( {
						message: xmld.getStringFromB64(0, 'msg')
					}
					);
				}
				//GridxPhonesF.Load();
		...
		GridxPhonesF.Delete = function() {
			...
			if(num>0) {
				...
				MH.notification.notify( {
					message: 'No hay remitentes seleccionados'
				}
				);
			}
		}
		...
		dojo.connect(GridxListContact.select.row, 'onSelectionChange', function(selected) {
			...
			var i = 0;
			while(i<numsel) {
				GridxListContact.store.fetch( {
					query: {
						unique_id: selected[i]
					}
					, onItem: function(item) {
						GridxListContact.RowSelected[i] = {
							idphone: GridxListContact.store.getValue(item, 'idphone'), name: GridxListContact.store.getValue(item, 'name'), phone: GridxListContact.store.getValue(item, 'phone')
						}
					}
				}
				);
			...
			if(GridxListContact.RowSelected.length>0) {
				...
				IFWS_4.fetch( {
					...
					, onItem: function(item) {
						...
						if(idphone) {
							items.push( {
								idphone: idphone, name: IFWS_4.getValue(item, 'name'), phone: IFWS_4.getValue(item, 'phone')
							}
							);
						}
					}
		...
		dijit.byId('idDialogDeletContactToSend').innerHTML('<div style="height: auto; width: 200px;">Está seguro de eliminar los contactos seleccionados?</div>').dijitOwner(dijit.byId('idBtnDeleteContact'), 'Click').on('onok', function() {
			...
			IFWS_4.fetch( {
				...
				, onItem: function(item) {
					if(!dojo.some(GridxListToSend.RowSelected, function(reg) {
						return reg == IFWS_4.getValue(item, 'unique_id');
					}
					)) {
						item.unique_id = i;
						items_.push(item);
		...
		GridxListToSend._setData = function(_items_) {
			...
			store.query(null, {
				...
				, {
					attribute: "phone", descending: false
				}
				]
			}
			...
			IFWS_4.data = {
				identifier: "unique_id", items: items_
			}
			;
			IFWS_4.close();
